...
predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 7+1, Narcotics=0)
predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 7, Narcotics=0+1)
predict(glm1, df, type="response")
```

&emsp; &emsp; ■ 兩個自變數的勝率效果分別是？<br>

#OfficeVisits=1.085564,Narcotics=1.079286
```{r}
...
#OfficeVisits=0.00843,Narcotics=0.00781
```{r}
...
predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 18.5+1, Narcotics=3)
predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 18.5, Narcotics=3+1)
predict(glm1, df, type="response")
```
...
#OfficeVisits=0.01718,Narcotics=0.01594
```{r}
0.30665-0.28947#OfficeVisits
0.30541-0.28947#Narcotics
```


&emsp; ■ 比較以上兩個題目的答案，我們可以觀察到什麼？<br><br>

#個變數對於結果的勝率效果不會改變,但個別變數的機率效果會改變
</p class="qiz">
