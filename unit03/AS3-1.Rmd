...
```{r}
#讀取資料框
Crime=read.csv("mvtWeek1.csv",stringsAsFactors = F)
```
...
```{r}
sum(Crime$LocationDescription=="ALLEY")
```
...
```{r}
ts = as.POSIXct(Crime$Date, format="%m/%d/%y %H:%M")
```
...
#2002-2008有下降趨勢
#2008-2012有微幅上升
hist(ts,'year',col="red",freq=T,las=2,cex.axis=0.7)
```
...
```{r}
table(Crime$Arrest,format(ts,'%y'))%>%prop.table(2)%>%round(3)
#0.104
```
...
```{r}
table(Crime$LocationDescription)%>%sort(decreasing = T)%>%head(6)
```

【4.2】 How many observations are in Top5?
```{r}
top5=names(table(Crime$LocationDescription)%>%sort(decreasing = T)%>%head(6))[-3]
sum(Crime$LocationDescription%in%top5)
```

【4.3】 One of the locations has a much higher arrest rate than the other locations. Which is it? 
```{r}
Top5=subset(Crime,LocationDescription%in%top5)
tapply(Top5$Arrest,Top5$LocationDescription,mean)%>%sort(decreasing =T)
#GAS STATION has much higher arrest rate
```

【4.4】 On which day of the week do the most motor vehicle thefts at gas stations happen?
```{r}
ts[Crime$LocationDescription=="GAS STATION"]%>%format('%u')%>%table%>%sort(decreasing = T)
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
ts[Crime$LocationDescription=="DRIVEWAY - RESIDENTIAL"]%>%format('%u')%>%table%>%sort(decreasing = F)
```
